...
- **Combine multiple sources into one collection** by passing an array - every collection-building call (`defineIconCollection`, `liveIconCollections`' values, the loader factories) accepts `IconSource | IconSource[]`. Each requested icon is resolved by trying sources in order, first match wins:
  ```ts
  import { defineIconCollection, iconify } from "astro-icon/collections";
