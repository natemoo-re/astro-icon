...
The same pattern works for a live collection: export the result of `defineLiveIconCollections({ "my-lib-live": source })` for a consumer to spread into their own `src/live.config.ts`.
