...
- **Pass `localSource` a `URL`, not a plain string, for bundled `.svg` files.** A plain relative string resolves against the *consuming* project's root (`config.root`), which is correct for a directory the consumer owns but wrong for one that ships inside your package. A `URL` resolves directly, so `localSource(new URL("../icons/", import.meta.url))` always points at the icons next to your own source file, no matter who imports it.
