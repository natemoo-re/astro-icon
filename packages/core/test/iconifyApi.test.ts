...
// `pack.ts` (used internally by `iconifyApi`) caches resolved packs in a module-level Map.
...
let iconifyApi: (typeof import("../src/content/iconify/iconifyApi.js"))["iconifyApi"];

beforeEach(async () => {
  vi.resetModules();
  ({ iconifyApi } = await import("../src/content/iconify/iconifyApi.js"));
});
...
describe("iconifyApi naming", () => {
  it("namespaces the source name with the pack", () => {
    expect(iconifyApi("mdi", { allowed: ["search"] }).name).toBe(
      "iconify-api:mdi",
    );
  });
});

describe("iconifyApi / host", () => {
...
describe("iconifyApi / batches an allowlist into one request", () => {
  it("resolves every allowed icon from a single fetch covering the whole allowlist", async () => {
    ...
    const source = iconifyApi("mdi", { allowed: ["search", "menu"] });
...
describe("iconifyApi / batches a single getIcons call, even without an allowlist", () => {
...
describe("iconifyApi / without an icons allowlist (e.g. <LiveIcon> against an uninstalled pack)", () => {
...
describe("iconifyApi / transform", () => {
  it("applies transform to the built entry, last, before it's returned", async () => {
    ...
    const source = iconifyApi("tabler", {
      allowed: ["search"],
      transform: (entry) => ({
        ...entry,
...
describe("iconifyApi / failure modes", () => {
...
describe("iconifyApi / pack cache sharing", () => {
  it("shares an API fetch across separate iconifyApi() instances requesting the same icon", async () => {
    const fetchMock = fetchReturning(() => pack);
    vi.stubGlobal("fetch", fetchMock);

    await iconifyApi("mdi", { allowed: ["search"] }).getIcons(["search"]);
    await iconifyApi("mdi", { allowed: ["search"] }).getIcons(["search"]);
