...
| ------- | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| **UC1** | A handful of local `.svg` files, no options                         | `localIcons()` with every default left alone.                                                                              | [site-builder](./site-builder) `src/content.config.ts`                                                                           |
...
| **UC6** | A library shipping its own icons into a consumer it doesn't control | `localIcons(new URL(...))`, anchored to the library's own module, plus a namespaced collection key.                        | [library-author](./library-author)                                                                                               |
...
| `localIcons()`                                            | ✅                                    | ✅ (brand)                     | ✅ (consumer + library)    |                                |
...
- **Composing several local directories or sources into one collection** (`createIconLoader([localIcons("a"), localIcons("b")])`, or a local source merged with an Iconify one via `mergeSources`). No persona's story needs this on its own terms, so it stays a listed gap rather than forced into one.
