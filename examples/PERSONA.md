...
| **UC3** | Icon names computed from data, drawn from a small known set         | Still a build-time collection - type the field as `IconName` and each value is checked where the data is defined.        | [platform-builder](./platform-builder) `flat-color-icons` collection |
