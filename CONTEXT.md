...
The `data-icon="collection:name"` attribute every `<Icon>`-rendered `<svg>` carries, regardless of context. A styling hook (`[data-icon="..."]` selectors) and the one thing that identifies astro-icon's output in a finished page. See `packages/core/components/Icon.astro`.
...
Nothing enforces that a collection's key, its source's `name`, and any underlying pack name line up - `iconifyLocalSource("mdi")` keyed as collection `mdi` is convention, not a rule. The demo's `combined` collection (`createIconLoader([iconifyLocalSource("fe"), iconifyLocalSource("ri")])`) is the concrete case where they don't: one collection, two packs, two sources, no shared name at all. This is a real "gotcha you'll hit the first time you try to merge two packs into one collection," not just terminology pedantry. A source's `name` is a diagnostic label only, keying nothing. For a live collection the key lives in exactly one declared place: `createLiveIconLoader`'s required `collection` option, which typegen keys `LiveCollectionName` off (Astro only reveals the real key per request, so the loader verifies the declared key against `context.collection` on first request and warns/re-records on mismatch). `liveIconCollections` (`content/liveCollections.ts`) removes even that one duplication by using its own object keys as both the registration key and the `collection` option.
